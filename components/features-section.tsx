...
const AnimatedChatDemo = ({ isActive }: { isActive: boolean }) => {
  const [messages, setMessages] = useState([
    { text: "Bonjour ! Comment puis-je vous aider aujourd'hui ?", isBot: true, visible: false },
    { text: "Je voudrais m'inscrire à une formation", isBot: false, visible: false },
    { text: "Parfait ! Je peux vous aider. Quelle formation vous intéresse ?", isBot: true, visible: false },
  ])
  ...
  useEffect(() => {
    ...
    const scenarios = [
      [
        ...
        { text: "Je voudrais m'inscrire à une formation", isBot: false },
        { text: "Parfait ! Je peux vous aider. Quelle formation vous intéresse ?", isBot: true },
      ],
      [
        { text: "Bonjour ! Je suis disponible 24/7 pour vous assister.", isBot: true },
        { text: "Proposez-vous des cours le week-end ?", isBot: false },
        { text: "Je peux vérifier nos disponibilités pour le week-end.", isBot: true },
      ],
      [
        { text: "Bonsoir ! Comment puis-je vous aider ?", isBot: true },
        { text: "J'ai besoin d'informations sur les tarifs", isBot: false },
        { text: "Je serais ravi de vous fournir les informations tarifaires immédiatement !", isBot: true },
      ],
    ]
          ...
          >
            <div
              className={`max-w-[80%] px-3 py-1.5 rounded-full text-xs ${
                msg.isBot ? "bg-slate-200 text-slate-700" : "bg-[#04a3fe] text-white"
              }`}
            >
              {msg.text}
            </div>
...
const AnimatedPhoneDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32 flex items-center justify-center relative">
      <div className="absolute top-2 right-2 text-xs text-slate-500 font-medium">Appels: {callCount + 1}</div>
      <div className="relative">
        ...
        {callState === "answered" && (
          <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
            <div className="bg-blue-100 px-2 py-1 rounded text-xs text-blue-700 whitespace-nowrap">Appel répondu</div>
          </div>
        )}
      </div>
    </div>
...
const AnimatedCalendarDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32">
            ...
            className={`w-4 h-4 flex items-center justify-center rounded transition-all duration-300 ${
              day === selectedDate
                ? booked
                  ...
                  : "bg-[#04a3fe] text-white scale-110"
                : day % 7 === 0 || day % 6 === 0
      ...
      {booked && (
        <div className="mt-2 text-xs text-green-600 font-medium animate-fade-in">✓ Séance réservée pour le 15</div>
      )}
    </div>
  )
}
...
const AnimatedEmailDemo = ({ isActive }: { isActive: boolean }) => {
  const [emails, setEmails] = useState([
    { subject: "Demande d'information", status: "unread" },
    { subject: "Inscription à la formation", status: "unread" },
    { subject: "Question sur le programme", status: "unread" },
  ])
    ...
    <div className="bg-slate-50 rounded-lg p-4 h-32 overflow-hidden">
      <div className="space-y-2">
        {emails.map((email, i) => (
          ...
          >
            <div className={`w-2 h-2 rounded-full ${email.status === "replied" ? "bg-green-500" : "bg-[#04a3fe]"}`} />
...
const AnimatedLeadsDemo = ({ isActive }: { isActive: boolean }) => {
  const [leads, setLeads] = useState([
    { name: "Ahmed M.", score: 0, qualified: false },
    { name: "Khaled D.", score: 0, qualified: false },
    { name: "Nassim R.", score: 0, qualified: false },
  ])
            ...
            <div className="flex-1 bg-slate-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${
                  lead.qualified ? "bg-green-500" : "bg-[#04a3fe]"
                }`}
...
const AnimatedIntegrationsDemo = ({ isActive }: { isActive: boolean }) => {
  const [connections, setConnections] = useState([
    { name: "Plateforme LMS", connected: false },
    { name: "LinkedIn", connected: false },
    { name: "Calendrier", connected: false },
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32">
      ...
      <div className="mt-2 text-center">
        <div className="text-xs text-slate-500">{connections.filter((c) => c.connected).length}/4 connectés</div>
      </div>
    </div>
  )
}
...
const features = [
  {
    title: "Support Pédagogique 24/7",
    description:
      "Formateurs dédiés qui répondent aux questions des étudiants, fournissent des conseils personnalisés et aidentent à leur progression tout au long de leur parcours d'apprentissage.",
  ...
  },
  {
    ...
    description:
      "Équipe de support disponible pour répondre aux appels, prendre des messages et planifier des séances d'information lorsque vous êtes occupé ou en dehors des heures de bureau.",
  ...
  },
  {
    ...
    description:
      "Système de planification qui vérifie les disponibilités des formateurs, réserve les séances et envoie des confirmations automatiquement.",
    demo: AnimatedCalendarDemo,
    size: "medium",
  },
  {
    title: "Réponse Email Automatisée",
    description:
      "Assistant email qui répond aux demandes, fournit des informations sur les programmes et transfère les questions complexes aux formateurs concernés.",
  ...
  },
  {
    ...
    description:
      "Système qui suit la progression des étudiants, identifie les difficultés et propose des ressources supplémentaires pour renforcer leurs compétences.",
  ...
  },
  {
    ...
    description:
      "Connectez-vous avec vos outils existants, y compris CRM, calendrier, WhatsApp, SMS, et plus pour une expérience d'apprentissage unifiée.",
          ...
          >
            <div className="inline-flex items-center px-4 py-2 rounded-full bg-slate-100 border border-slate-200 text-slate-700 text-sm font-medium mb-6">
              <svg className="w-4 h-4 mr-2 text-slate-600" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2C13.1 2 14 2.9 14 4C14 5.1 13.1 6 12 6C10.9 6 10 5.1 10 4C10 2.9 10.9 2 12 2ZM21 9V7L15 1H5C3.89 1 3 1.89 3 3V7H1V9H3V15H3V17H3V21C3 22.11 3.89 23 5 23H19C20.11 23 21 22.11 21 21V17H23V15H21V9H23ZM19 9V15H5V9H19ZM7.5 11.5C7.5 10.67 8.17 10 9 10S10.5 10.67 10.5 11.5 9.83 13 9 13 7.5 12.33 7.5 11.5ZM13.5 11.5C13.5 10.67 14.17 10 15 10S16.5 10.67 16.5 11.5 15.83 13 15 13 13.5 12.33 13.5 11.5ZM12 16C13.11 16 14.08 16.59 14.71 17.5H9.29C9.92 16.59 10.89 16 12 16Z" />
              </svg>
              Formation Continue - Développez Votre Carrière
            </div>
            ...
            <p className="text-base sm:text-lg md:text-xl text-slate-600 max-w-3xl mx-auto font-light leading-relaxed">
              Découvrez comment notre équipe pédagogique vous accompagne tout au long de votre parcours d'apprentissage, de l'inscription à l'emploi.
            </p>
          </div>
