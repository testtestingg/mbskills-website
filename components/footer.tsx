    ...
    <footer className="md:rounded-t-6xl relative w-full max-w-6xl mx-auto flex flex-col items-center justify-center rounded-t-4xl border-t bg-[radial-gradient(35%_128px_at_50%_0%,theme(backgroundColor.white/8%),transparent)] px-6 py-12 lg:py-16">
      ...
      <div className="grid w-full gap-8 xl:grid-cols-3 xl:gap-8">
        <AnimatedContainer className="space-y-4">
          <Image src="/images/mbskills-logo.png" alt="MBSkills Logo" width={64} height={64} className="size-16" />
          <div className="text-muted-foreground mt-8 text-sm md:mt-0 md:block hidden">
            <p>© {new Date().getFullYear()} MBSkills. Tous droits réservés.</p>
          </div>
        </AnimatedContainer>
      ...
      <div className="md:hidden mt-8 text-center space-y-2">
        <p className="text-muted-foreground text-sm">© {new Date().getFullYear()} MBSkills. Tous droits réservés.</p>
        <p className="text-muted-foreground text-xs">Développé par MBSkills Tech</p>
      </div>

      <div className="hidden md:block mt-8 pt-6 border-t border-foreground/10 w-full">
        <p className="text-muted-foreground text-xs text-center">Développé par MBSkills Tech</p>
      </div>
    </footer>
  )
}
